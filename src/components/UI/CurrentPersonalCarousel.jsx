...
const CurrentPersonalCarousel = () => {
    const { currentPersonal } = profileData;
    const [currentIndex, setCurrentIndex] = useState(0);
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrentIndex((prevIndex) =>
                prevIndex === currentPersonal.length - 1 ? 0 : prevIndex + 1
            );
        }, 10000);
    ...
    const personal = currentPersonal[currentIndex];
    ...
    return (
        <div className="w-full max-w-sm mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="p-6">
                ...
                {/* Navigation Dots */}
                <div className={`flex justify-center space-x-2 mb-3 ${currentPersonal.length <= 1 ? 'invisible' : ''}`}>
                    {currentPersonal.map((_, index) => (
                        <button
                            key={index}
            ...
            <div className="h-1 bg-gray-200">
                <div
                    ...
                    style={{
                        width: `${((currentIndex + 1) / personal.length) * 100}%`
                    }}
                />
            </div>
        </div>
...
export default CurrentPersonalCarousel;